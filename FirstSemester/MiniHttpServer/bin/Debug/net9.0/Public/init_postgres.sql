-- 1. Убеждаемся, что схема public есть
CREATE SCHEMA IF NOT EXISTS public;

-- 2. Удаляем старые таблицы (если они криво создались), чтобы создать начисто
-- 1. Очистка
...
-- Варианты номеров (Room Selection)
...
-- Удобства (разбитые по категориям)
CREATE TABLE public."amenities" (
                                    "Id" SERIAL PRIMARY KEY,
                                    "TourId" INT,
                                    "Category" TEXT, -- 'General', 'Location', 'InHotel', 'Pools', 'Beach'
                                    "Title" TEXT,
                                    "Content" TEXT
);
...
CREATE TABLE public."users" (
                                "Id" SERIAL PRIMARY KEY,
                                "Email" TEXT,
                                "Password" TEXT, -- В учебных целях храним просто текст (или простой хеш)
...
-- Погода (заполняем пару месяцев для примера)
...
-- Свободные даты (из вашего дампа)
...
-- ДРУГИЕ ОТЕЛИ (для блока "Похожие отели")
/*INSERT INTO public."tours" ("Name", "Location", "Image", "Price", "Date", "Duration", "MealPlan", "IsEarlyBooking", "IsDiscount") VALUES
                                                                                                                                      ('GLORIA VERDE RESORT', 'Турция, Белек', 'hotels/gloria.jpg', '164 142,19 ₽', '13.12.2025', '6 ночей', 'Ультра все включено', true, false),
                                                                                                                                      ('Limak Arcadia', 'Турция, Белек', 'hotels/limak.jpg', '110 000 ₽', '15.12.2025', '7 ночей', 'Все включено', false, true);
*/
